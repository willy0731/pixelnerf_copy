...
def extra_args(parser):
    parser.add_argument("--subset", "-S", type=int, default=0, help="Subset in data to use")
...
if __name__ == '__main__':
    ...
    dtu_format = hasattr(dataset, "sub_format") and dataset.sub_format == "dtu"
    if dtu_format:
        print("Using DT camera trajectory")
    ...
    render_rays = util.gen_rays(render_poses, W, H, focal*args.scale, z_near, z_far, c=c*args.scale if c is not None else None).to(device=device) # [NV,H,W,8]
    focal = focal.to(device=device)
    source = torch.tensor(list(map(int, args.source.split())), dtype=torch.long) # [64]
    NS = len(source) # 1
    random_source = NS == 1 and source[0] == -1 # Tensor(False)
    ...
    with torch.no_grad():
        ...
        if random_source: # Not default
            src_view = torch.randint(0,NV, (1,))
        else:
            src_view = source
        
        model.encode(images[src_view].unsqueeze(0), poses[src_view].unsqueeze(0).to(device=device), focal, c=c)
        ...
        _depth = None
        rgb_fine = torch.cat(all_rgb_fine) # [V*H*W, 3]
    ...
    viewimg_path = os.path.join(args.visual_path, args.name, "video" + vid_name + "_view.jpg")
    imageio.mimwrite(vid_path, (frames.cpu().numpy() * 255).astype(np.uint8), fps=args.fps, quality=8)
    ...
    print("Write into", vid_path, "view:", viewimg_path)
