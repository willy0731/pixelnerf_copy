from torch import nn
import torch

#  import torch_scatter
import torch.autograd.profiler as profiler
import util

# ResnetFC中的self.blocks 主要作Residual的地方
class ResnetBlockFC(nn.Module):
    """
    Fully connected ResNet Block class.
    Taken from DVR code.
    :param size_in (int): input dimension
    :param size_out (int): output dimension
    :param size_h (int): hidden dimension
    """

    def __init__(self, size_in, size_out=None, size_h=None, beta=0.0):
        super().__init__()
        # Attributes
        if size_out is None:
            size_out = size_in

        if size_h is None: # 取 input, output較小的當hidden size
            size_h = min(size_in, size_out)

        self.size_in = size_in
        self.size_h = size_h
        self.size_out = size_out
        # Submodules
        self.fc_0 = nn.Linear(size_in, size_h)
        self.fc_1 = nn.Linear(size_h, size_out)

        # Init
        nn.init.constant_(self.fc_0.bias, 0.0)
        nn.init.kaiming_normal_(self.fc_0.weight, a=0, mode="fan_in")
        nn.init.constant_(self.fc_1.bias, 0.0)
        nn.init.zeros_(self.fc_1.weight)

        if beta > 0:
            self.activation = nn.Softplus(beta=beta) # softplus(x)=log(1+exp(x))
        else:
            self.activation = nn.ReLU()

        if size_in == size_out:
            self.shortcut = None
        else: # 若input output size不同 則
            self.shortcut = nn.Linear(size_in, size_out, bias=False)
            nn.init.constant_(self.shortcut.bias, 0.0)
            nn.init.kaiming_normal_(self.shortcut.weight, a=0, mode="fan_in")

    def forward(self, x):
        with profiler.record_function("resblock"):
            net = self.fc_0(self.activation(x)) # 將x作activate後丟進Linear layer->size_h
            dx = self.fc_1(self.activation(net)) # 將net作activate後丟進Linear layer->size_out

            if self.shortcut is not None:
                x_s = self.shortcut(x)
            else:
                x_s = x
            return x_s + dx

# PixelNeRF的MLP
class ResnetFC(nn.Module):
    def __init__(
        self,
        d_in,
        d_out=4,
        n_blocks=5, # 論文中default=3
        d_latent=0,
        d_hidden=128, # 論文在Coarse/fine default=512
        beta=0.0,
        combine_layer=1000, # 論文中default=3
        combine_type="average",
        use_spade=False,
    ):
        """
        :param d_in input size
        :param d_out output size
        :param n_blocks number of Resnet blocks
        :param d_latent latent size, added in each resnet block (0 = disable)
        :param d_hidden hiddent dimension throughout network
        :param beta softplus beta, 100 is reasonable; if <=0 uses ReLU activations instead
        """
        super().__init__()
        if d_in > 0:
            # 把PE好的座標&viewdir丟進一層lineay layer
            self.lin_in = nn.Linear(d_in, d_hidden)
            nn.init.constant_(self.lin_in.bias, 0.0)
            nn.init.kaiming_normal_(self.lin_in.weight, a=0, mode="fan_in")

        self.lin_out = nn.Linear(d_hidden, d_out) # 轉成RGB,sigma 的最後一層layer
        nn.init.constant_(self.lin_out.bias, 0.0)
        nn.init.kaiming_normal_(self.lin_out.weight, a=0, mode="fan_in")

        self.n_blocks = n_blocks # 論文 default_mv.conf=5
        self.d_latent = d_latent # 0
        self.d_in = d_in
        self.d_out = d_out
        self.d_hidden = d_hidden # 論文在Coarse/fine default=512

        self.combine_layer = combine_layer # default_mv.conf = 3
        self.combine_type = combine_type # average
        self.use_spade = use_spade # default=False

        self.blocks = nn.ModuleList( # beta=0, 
            [ResnetBlockFC(d_hidden, beta=beta) for i in range(n_blocks)]
        )

        if d_latent != 0: # latent_feature
            n_lin_z = min(combine_layer, n_blocks) # min(3,3) => 通常會是n_blocks=3
            self.lin_z = nn.ModuleList( # Linear layer *3
                [nn.Linear(d_latent, d_hidden) for i in range(n_lin_z)] 
            )
            for i in range(n_lin_z): # 初始化這三層linear layer的weights和bias
                nn.init.constant_(self.lin_z[i].bias, 0.0)
                nn.init.kaiming_normal_(self.lin_z[i].weight, a=0, mode="fan_in")

            if self.use_spade:
                self.scale_z = nn.ModuleList(
                    [nn.Linear(d_latent, d_hidden) for _ in range(n_lin_z)]
                )
                for i in range(n_lin_z):
                    nn.init.constant_(self.scale_z[i].bias, 0.0)
                    nn.init.kaiming_normal_(self.scale_z[i].weight, a=0, mode="fan_in")

        if beta > 0:
            self.activation = nn.Softplus(beta=beta)
        else:
            self.activation = nn.ReLU()

    def forward(self, zx, combine_inner_dims=(1,), combine_index=None, dim_size=None):
        """
        :param zx (..., d_latent + d_in)
        :param combine_inner_dims Combining dimensions for use with multiview inputs.
        Tensor will be reshaped to (-1, combine_inner_dims, ...) and reduced using combine_type
        on dim 1, at combine_layer
        """
        with profiler.record_function("resnetfc_infer"):
            assert zx.size(-1) == self.d_latent + self.d_in
            if self.d_latent > 0:
                z = zx[..., : self.d_latent] # latent feature
                x = zx[..., self.d_latent :] # location(PE) and viewdirs
            else:
                x = zx
            if self.d_in > 0:
                x = self.lin_in(x)
            else:
                x = torch.zeros(self.d_hidden, device=zx.device)

            for blkid in range(self.n_blocks): # n_blocks = 5
                if blkid == self.combine_layer: # 第3層結束 n=3的時候 (準備進第4層前)
                    x = util.combine_interleaved(x, combine_inner_dims, self.combine_type)# 取Mean

                # 所以latent feature 會丟進去三次
                if self.d_latent > 0 and blkid < self.combine_layer: 
                    tz = self.lin_z[blkid](z) # tz=將latent feature丟進linear layer
                    if self.use_spade: # default=False 通常不會跑 論文中沒使用SPADE
                        sz = self.scale_z[blkid](z) # sz=將latent feature丟進linear layer
                        x = sz * x + tz # 計算使用SPADE下處理後的latent feature
                    else:
                        x = x + tz # 預設使用這個 把各經一層Linear layer的x,z加起來

                x = self.blocks[blkid](x)
            out = self.lin_out(self.activation(x))
            return out

    @classmethod
    def from_conf(cls, conf, d_in, **kwargs):
        # PyHocon construction
        return cls(
            d_in,
            n_blocks=conf.get_int("n_blocks", 5),
            d_hidden=conf.get_int("d_hidden", 128),
            beta=conf.get_float("beta", 0.0),
            combine_layer=conf.get_int("combine_layer", 1000),
            combine_type=conf.get_string("combine_type", "average"),  # average | max
            use_spade=conf.get_bool("use_spade", False),
            **kwargs
        )
