...
class MultiObjectDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path, stage="train", z_near=4, z_far=9, n_views=None):
        super().__init__()
        path = os.path.join(path, stage) # data/"stage"
        self.base_path = path
        print("Loading NeRF synthetic dataset", self.base_path)
        trans_files = []
        TRANS_FILE = "transforms.json"
        for root, directories, filenames in os.walk(self.base_path): # 抓所有data
    ...
    def _check_valid(self, index): # 檢查dataset是否正常
    ...
    def __getitem__(self, index):
        if not self._check_valid(index): # not default
            return {}
