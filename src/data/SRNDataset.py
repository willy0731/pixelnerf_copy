...
class SRNDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path, stage="train", image_size=(128, 128), world_scale=1.0):
        ...
        if path == 'src/data':
            ...
            self.base_path = path + "_" + stage # data/srn_cars_train|val|test
        ...
        if is_chair and stage == "train": # 修改chair的路徑
        ...
        self.intrins = sorted(glob.glob(os.path.join(self.base_path, "*", "intrinsics.txt"))) 
            # "*" 代表不管中間資料夾名稱, 把intrinsics.txt按照順序排列
        ...
        self._coord_trans = torch.diag( torch.tensor([1, -1, -1, 1], dtype=torch.float32)) # 把y,z軸加上負號
