...
def get_split_dataset(dataset_type, datadir, want_split="all", training=True, **kwargs):
    ...
    flags, train_aug_flags = {}, {} # flags是根據特定dataset的補充標籤
    ...
    # 在指定情況下將對應設定為True 來對應split種類 若都不是 則接為True 三者皆要
    ...
    if want_train:
        ...
        if train_aug is not None: # default 不跑
    ...
    #　因為這些dataset都分好train, val, test的資料夾了
    if want_split == "train":
        return train_set
    elif want_split == "val":
        return val_set
        ...
        return test_set
    return train_set, val_set, test_set # want_split="all" 傳回所有
    #　but 訓練時用不到test_set
