...
class ColorJitterDataset(torch.utils.data.Dataset):
    def __init__(self,base_dset,hue_range=0.1,saturation_range=0.1,brightness_range=0.1,contrast_range=0.1,extra_inherit_attrs=[],):
    ...
    def apply_color_jitter(self, images):
        ...
        for i in range(len(images)):
            ...
            tmp = F_t.adjust_saturation(tmp, saturation_factor) # 調整飽和度
            tmp = F_t.adjust_hue(tmp, hue_factor) # 調整色調
            tmp = F_t.adjust_contrast(tmp, contrast_factor) # 調整對比度
            tmp = F_t.adjust_brightness(tmp, brightness_factor) # 調整亮度
