...
class AlphaLossNV2(torch.nn.Module):
    ...
    def __init__(self, lambda_alpha, clamp_alpha, init_epoch, force_opaque=False):
        ...
        self.force_opaque = force_opaque # 強制不透明
        if force_opaque:
            self.bceloss = torch.nn.BCELoss() # Cross-entropy
    ...
    def forward(self, alpha_fine):
        if self.lambda_alpha > 0.0 and self.epoch.item() >= self.init_epoch:
            alpha_fine = torch.clamp(alpha_fine, 0.01, 0.99) # 將alpha限於0~1
            if self.force_opaque:
                # 使用cross-entropy計算loss後乘上權重(lambda_alpha)
                ...
                alpha_loss = torch.clamp_min(alpha_loss, -self.clamp_alpha) # 確保負數不會過大
                alpha_loss = self.lambda_alpha * alpha_loss.mean() # 取平均
        else:
...
def get_alpha_loss(conf):
    lambda_alpha = conf.get_float("lambda_alpha") # 0.0
    clamp_alpha = conf.get_float("clamp_alpha") # 100
    init_epoch = conf.get_int("init_epoch") # 5
...
class RGBWithUncertainty(torch.nn.Module):
    ...
    def forward(self, outputs, targets, betas):
        ...
        weighted_element_err = ( # 把誤差除以相應權重
...
def get_rgb_loss(conf, coarse=True, using_bg=False, reduction="mean"):
    if conf.get_bool("use_uncertainty", False) and not coarse:
        print("using loss with uncertainty") # 針對uncertainty data才使用
    ...
    return ( # 通常return這個
        ...
        if conf.get_bool("use_l1")
        else torch.nn.MSELoss(reduction=reduction) # default this
    )
