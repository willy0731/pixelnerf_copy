...
class PixelNeRFTrainer(trainlib.Trainer):
    def __init__(self):
        ...
        self.renderer_state_path = "%s/%s/_renderer" % ( # checkpoint
        ...
        # rgb_coarse/fine_crit : loss function
...
# 依照現在的step數呼叫NeRFRenderer中的sched_step更新sample數
    def post_batch(self, epoch, batch):
        renderer.sched_step(args.batch_size)

    def extra_save_state(self): # 儲存現在的logs
    ...
    def calc_losses(self, data, is_train=True, global_step=0):
        ...
        curr_nviews = nviews[torch.randint(0, len(nviews), ()).item()] # 要拿來訓練的view
        if curr_nviews == 1:
            image_ord = torch.randint(0, NV, (SB, 1)) # 隨機選一張
        else:
            image_ord = torch.empty((SB, curr_nviews), dtype=torch.long) # 選指定張數
        for obj_idx in range(SB):
            if all_bboxes is not None:
                bboxes = all_bboxes[obj_idx] # 獲取邊界資訊
            ...
            rgb_gt_all = images_0to1 # [NV,3,H,W]
            rgb_gt_all = (
                rgb_gt_all.permute(0, 2, 3, 1).contiguous().reshape(-1, 3)
            )  # (NV, H, W, 3) 應該是變成 [NV*H*W, 3]

            # 這邊是在生成訓練pixel的索引 用來排訓練順序
            if all_bboxes is not None:
                pix = util.bbox_sample(bboxes, args.ray_batch_size)
                # index*H*W + y*W + x = 一維pixel array中的index位置
            ...
            rays = cam_rays.view(-1, cam_rays.shape[-1])[pix_inds].to(
                device=device # 把要訓練的pixel照index順序排好
            )  # (ray_batch_size, 8) ; ray_batch_size=NV*H*W
        ...
        image_ord = image_ord.to(device) # 這是選定要來訓練的
        src_images = util.batched_index_select_nd( # 找出相對應索引的圖片
            all_images, image_ord
        )  # (SB, NS, 3, H, W)
        # 同理如上
        src_poses = util.batched_index_select_nd(all_poses, image_ord)  # (SB, NS, 4, 4)

        all_bboxes = all_poses = all_images = None # 將這些歸零

        net.encode( # 丟進去encoder裡
            src_images,
            src_poses,
        ...
        rgb_loss = self.rgb_coarse_crit(coarse.rgb, all_rgb_gt) # 計算coarse_loss
        loss_dict["rc"] = rgb_loss.item() * self.lambda_coarse # "rc" 存取乘權重的loss
        if using_fine:
            fine_loss = self.rgb_fine_crit(fine.rgb, all_rgb_gt)
            rgb_loss = rgb_loss * self.lambda_coarse + fine_loss * self.lambda_fine
            loss_dict["rf"] = fine_loss.item() * self.lambda_fine # "rf" 存取乘權重的loss

        loss = rgb_loss
        if is_train:
            loss.backward() 
        loss_dict["t"] = loss.item() # "t"用來儲存最後的loss

        return loss_dict

    def train_step(self, data, global_step): # 呼叫calc_losses計算loss
        return self.calc_losses(data, is_train=True, global_step=global_step)

    def eval_step(self, data, global_step): # 計算val_loss
        renderer.eval()
