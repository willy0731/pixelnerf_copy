...
class Trainer:
    def __init__(self, model, train_dataset, test_dataset, args, conf, device=None):
        ...
        self.train_data_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=False)
        self.test_data_loader = torch.utils.data.DataLoader(test_dataset, batch_size=min(args.batch_size, 16), shuffle=True, num_workers=4, pin_memory=False)
        # Setting parameters
        self.num_total_batches = len(self.train_dataset) # 類別數
        self.exp_name = args.name
        self.save_interval = conf.get_int("save_interval") # 50
        self.print_interval = conf.get_int("print_interval") # 2
        self.vis_interval = conf.get_int("vis_interval") # 100
        self.eval_interval = conf.get_int("eval_interval") # 50
        self.num_epoch_repeats = conf.get_int("num_epoch_repeats", 1) # 1, 訓練DTU的時候要調整
        self.num_epochs = args.epochs
        self.accu_grad = conf.get_int("accu_grad", 1) # 1, disable
        ...
        if args.gamma != 1.0:
            self.lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.optimizer, gamma=args.gamma)
        else: # default
        ...
        if args.resume:
            ...
            if self.lr_scheduler is not None and os.path.exist(self.lrsched_state_path):
            ...
            if not self.managed_weight_saving and os.path.exist(self.default_net_state_path):
                model.load_state_dict(torch.load(self.default_net_state_path, map_location=device))

        self.visual_path = os.path.join(self.args.visual_path, self.args.name) # visuals/exp_name
        self.conf = conf
            ...
            for _ in range(self.num_epoch_repeats):
                for data in self.train_data_loader:
                    ...
                    if batch % self.save_interval == 0 and (epoch > 0 or batch > 0):
                        print("Saving...")
                        if self.managed_weight_saving:
                            self.model.save_weights(self.args)
                        else:
                            torch.save(self.model.state_dict(), self.default_net_state_path)
                    ...
                    self.post_batch(epoch, batch) # default 不跑                 
                    step_id += 1
                    batch += 1
                    progress.update(1)
            if self.lr_scheduler is not None:
